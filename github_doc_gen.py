...
It includes strict rate limit checking to prevent execution when GitHub API rate limits are low.
"""
...
logger = logging.getLogger('github_doc_gen')
...
def check_github_rate_limit() -> None:
    """
    Check GitHub API rate limit status and exit if too low
    
    This function will exit the program if the rate limit is too low.
    """
    headers = {}
    if GITHUB_TOKEN:
...
def fetch_raw_content(owner: str, repo: str, path: str, branch: str = "main") -> str:
    ...
    try:
        ...
        if response.status_code == 429:
            ...
            print(f"\nERROR: GitHub API rate limit exceeded while fetching {raw_url}")
        ...
        if branch == "main":
            try:
                ...
                if response.status_code == 429:
                    ...
                    print(f"\nERROR: GitHub API rate limit exceeded while fetching {alternate_url}")
                    print("Please try again later when the rate limit resets.")
                    sys.exit(1)
...
def fetch_repo_tree(owner: str, repo: str, branch: str = "main") -> List[Dict[str, Any]]:
    ...
    try:
        ...
        if response.status_code == 429:
            ...
            print(f"\nERROR: GitHub API rate limit exceeded while fetching repo tree")
            print("Please try again later when the rate limit resets.")
            sys.exit(1)
...
def fetch_repo_data(owner: str, repo: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # Check if we hit rate limiting
        if response.status_code == 429:
            logger.error(f"Rate limit exceeded while fetching repo info")
            print(f"\nERROR: GitHub API rate limit exceeded while fetching repo info")
            print("Please try again later when the rate limit resets.")
            sys.exit(1)
...
def generate_docs(repo_url: str, output_dir: str = "output", open_docs: bool = True, skip_github: bool = False) -> None:
    ...
    if skip_github:
        ...
        if not repo_data:
            logger.error(f"No cached data available for {owner}/{repo} and --skip-github was specified")
            print(f"\nERROR: No cached data available for {owner}/{repo} and --skip-github was specified")
            print("Please run without --skip-github first to create a cache.")
            sys.exit(1)
